...
if [ ! -f /opt/pihole/credentials.ini ]; then
  ...
  echo
  read -s -p "Enter Email for SSL Cert: " CERBOT_Email
  echo
...
sudo certbot certonly \
  ...
  --email "jordan@rjproductionsut.com" \
  --agree-tos \
  --non-interactive \
  -d "$DNS_ADDRESS"
...
echo "Pi-hole with Nginx, SSL (Certbot or fallback self-signed), DNS over TLS, and HTTPS on port 443 setup completed!"
